...
export default function CatalogueClient({ products }: { products: Product[] }) {
  ...
  const filtered = active === 'All' ? products : products.filter(p => p.material === active)
      ...
      <section className="section" style={{ background: 'var(--canvas)' }}>
        <div className="container">
            ...
            {materials.map(mat => (
              <button
                ...
                style={{
                  ...
                  color: active === mat ? '#fff' : 'var(--ash)',
                  background: active === mat ? 'var(--brand)' : 'transparent',
                  cursor: 'pointer',
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--ash)', marginBottom: '1.5rem' }}>
            Showing {filtered.length} stove{filtered.length !== 1 ? 's' : ''}
          </p>

          {/* Grid */}
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '4rem 0', color: 'var(--ash)' }}>
              <p style={{ fontSize: '1.1rem' }}>No products in this category.</p>
            </div>
          ) : (
            <div style={{
              display: 'grid',
